  ...
  if (username && password) {
    ...
    if (customer) {
      if (customer.active === 1) {
        ...
        res.json({
          success: true, message: 'Authentication successful', token: token, customer: customer
        });
      } else {
...
router.post('/signup', async function (req, res) {
  ...
  if (dbCust) {
    ...
    const newCust = {
      username: username, password: password, name: name, phone: phone, email:
        email, active: 0, token: token
    };
...
router.get('/products', async function (req, res) {
  ...
  const products = await ProductDAO.selectBySkipLimit(skip, limit);
  res.json(products);
});
